...
export const LoginPage: React.FC = () => {
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            console.error(err);
            setError('שגיאה בהתחברות עם Google: ' + (err.message || 'אנא נסה שנית'));
        }
    };

    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-100" dir="rtl">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden">
                <div className="bg-blue-600 p-8 text-center">
                    ...
                    <h1 className="text-2xl font-bold text-white mb-2">ברוכים הבאים ל-BioShield</h1>
                    <p className="text-blue-100">מערכת ניטור עמידות לפונגצידים</p>
                </div>
                ...
                <div className="p-8 space-y-6">
                    ...
                    <p className="text-center text-slate-500 mb-4">
                        ההתחברות למערכת מתבצעת באמצעות חשבון Google בלבד.
                        <br />
                        <span className="text-xs text-slate-400">(רק משתמשים מאושרים יוכלו לגשת לממשקי הניהול)</span>
                    </p>

                    <button
                        type="button"
                        onClick={handleGoogleLogin}
                        className="w-full bg-white border-2 border-slate-100 text-slate-700 font-bold py-4 px-6 rounded-xl flex items-center justify-center gap-3 hover:bg-slate-50 hover:border-slate-200 transition-all shadow-sm hover:shadow-md"
                    >
                    ...
                    <div className="pt-4 border-t border-slate-100 text-center">
                        <button onClick={() => navigate('/')} className="text-slate-400 hover:text-blue-600 text-sm font-medium transition-colors">
                            חזרה לדף הבית (ללא התחברות)
                        </button>
                    </div>
                </div>
            </div>
